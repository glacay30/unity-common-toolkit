...
[ExecuteAlways]
public class FollowPath : MonoBehaviour, IResettable
{
    ...
    private void FixedUpdate()
    {
        if (Application.IsPlaying(gameObject)) {
            ...
            if (movementType == MovementType.ConstantSpeed) {
                transform.position = Vector3.MoveTowards(transform.position, nodesEnumerator.Current.position, Time.fixedDeltaTime * speed);
            }
